<template>
  <div class="container mt-4">
    <h2 class="text-primary mb-4">Editar Ingrediente Extra de Pedido</h2>
    <form @submit.prevent="updateExtraIngredient">
      ...
      <router-link to="/order_extra_ingredient" class="btn btn-secondary ms-2">Cancelar</router-link>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    loadData() {
      ...
      axios.get(`http://127.0.0.1:8000/api/order_extra_ingredient/${id}`)
        .then(res => {
          this.form = {
            order_id: res.data.orderExtraIngredient.order_id,
            extra_ingredient_id: res.data.orderExtraIngredient.extra_ingredient_id,
            quantity: res.data.orderExtraIngredient.quantity
          }
    ...
    updateExtraIngredient() {
      const id = this.$route.params.id
      axios.put(`http://127.0.0.1:8000/api/order_extra_ingredient/${id}`, this.form)
        .then(() => {
